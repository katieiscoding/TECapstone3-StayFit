...
public class WorkoutController {
    ...
    @RequestMapping(path= "/workout/generate", method = RequestMethod.POST)
    public List<ExerciseTrainer> generatedWorkout(@RequestParam(required = false) String creatorName, @RequestBody String[] bodyTargetIds, @RequestParam int totalTime){

        if(bodyTargetIds == null){
            bodyTargetIds = new String[0];
        }
        return workoutDAO.generateWorkout(creatorName, bodyTargetIds, totalTime);
    }
